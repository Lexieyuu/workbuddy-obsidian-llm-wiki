---
title: "摘要-workbuddy-official-skills"
type: source
status: verified
sources: []
source_url: "https://cloud.tencent.com/document/product/1831/134432"
raw_path: "raw/01-articles/workbuddy-official-skills.md"
author: "腾讯云官方文档"
published: "未知"
created: 2026-09-02
updated: 2026-09-02
tags: [workbuddy, skill, security]
---

# WorkBuddy 官方技能说明摘要

## 核心摘要

Skill 是 WorkBuddy 扩展工具能力和工作流程的方式，可以通过市场、本地技能包或自然语言创建。由于 Skill 可能读取本地文件或调用外部服务，安装前需要检查来源和权限。

## 关联连接

- [[WorkBuddy]] — 技能运行平台
- [[AgentSkills]] — Skill 的结构和作用
- [[摘要-codebuddy-official-skills-format]] — `SKILL.md` 格式

## 来源与引用

- [[raw/01-articles/workbuddy-official-skills.md]]
